function [C_Ave_Var1, C_Ave_Var2, C_Ave_Var3]=func_TrendAve_CellAreaWeighted_3var_grid(Var1, Var2, Var3, Lat, Lon, Lat_bound, Lon_bound, NO_st_d, min_NO_st_d)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Behzad Asadieh                                      %%%
...
GridCell_Areas3=GridCell_Areas1;

if nargin == 7 % Number of input arguments is less than 7 - That means the NO_st_d and min_NO_st_d are not given and so do not play important role in the process
...
GridCell_Areas3(isnan(Var3))=NaN;
        ...
        if NO_st_d(Lt,Ln)>= min_NO_st_d
            ...
            for i=1:size(sum_Var1,1)
                ...
                if (Lat(Lt,1) > Continents_LatLong_Bound(i,1)) && (Lat(Lt,1) <= Continents_LatLong_Bound(i,2)) && (Lon(Ln,1) > Continents_LatLong_Bound(i,3)) && (Lon(Ln,1) <= Continents_LatLong_Bound(i,4))
                    ...
                    if ~isnan(Var3(Lt,Ln))
                        sum_Var3(i,1)=sum_Var3(i,1)+Var3(Lt,Ln) * GridCell_Areas3 (Lt,Ln);
                    end
...
C_Ave_Var3(1,1)=nansum(nansum( Var3 .* GridCell_Areas3 )) / nansum(nansum( GridCell_Areas3 )); % Global
C_Ave_Var3(2,1)=sum_Var3(1,1)/area_accumulator(1,1); % North America
C_Ave_Var3(3,1)=sum_Var3(2,1)/area_accumulator(2,1); % South America
C_Ave_Var3(4,1)=sum(sum_Var3(4:5,1))/sum(area_accumulator(4:5,1)); % Europe
C_Ave_Var3(5,1)=sum_Var3(3,1)/area_accumulator(3,1); % Oceania
C_Ave_Var3(6,1)=sum(sum_Var3(14:15,1))/sum(area_accumulator(14:15,1)); % Africa
C_Ave_Var3(7,1)=sum(sum_Var3(6:13,1))/sum(area_accumulator(6:13,1)); % Asia
C_Ave_Var3(8,1)=sum(sum_Var3(12:13,1))/sum(area_accumulator(12:13,1)); % India
